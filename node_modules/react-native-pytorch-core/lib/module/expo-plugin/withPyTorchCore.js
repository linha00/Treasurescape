 ...
 * @format
 */
import { withAppBuildGradle, createRunOncePlugin } from '@expo/config-plugins'; // Keeping the name, and version in sync with it's package.
...
export function setClassPath(_config, buildGradle) {
  let mutatedBuildGradle = buildGradle.replace(/android\s?{/, `android {
    /**
...
`);
  mutatedBuildGradle = mutatedBuildGradle.replace(/dependencies\s?{/, `
