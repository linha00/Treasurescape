...
function MissionPage() {
    const nav = useRouter(); 
    const { user } = useAuth();
    const [missionId, setMissionId] = useState(0); //mission ID is zero for now
    const [text, setText] = useState("temp"); //text is temp
    const [gold, setGold] = useState(0); //gold number is 0
    
    const [loading, setLoading] = useState(false); //loading is false
    const [refresh, setRefresh] = useState(false); //refresh is false
    const [enter_prompt, setenter_prompt] = useState(false); //set enter prompt to false
    const [correct, setCorrect] = useState(false); //set correct to false
    const [wrong, setWrong] = useState(false); //set wrong to false
    const [enter_loading, setEnter_loading] = useState(false); //set enter loading to false
    
    const {control, handleSubmit } = useForm(); //handling form submission

  
    async function getMission() {
        setLoading(true);
        let {data} = await supabase.from('profiles').select().eq('id', user.id).single(); //get user profile
        let tempid = data.mission; //get mission number
        if (tempid != null) { //if there is mission available
            let {data} = await supabase.from('missions').select().eq('id', tempid).single(); //get mission info
            setGold(data.award); //current gold level
            setMissionId(data.id); //current user id
    ...
    const enter_pressed = async input => {
        const keyed = input.passcode; //getting the passcode inputted by users 
        ...
        try {
            ...
            if (mission != 0) {
                ...
                if (passcod == keyed) { //if the passcode is correct
                    if (reward != 0 && mission != 4) { //if it is not mission 4
                        await supabase
                        .from('profiles')
                        .update({ 
                            gold: balance + reward,
        ...
        <>
            <SafeAreaView style={styles.container}>
                ...
                <View style={styles.button}>
                    ...
                    <Modal //first modal is for prompt to enter passcode
                        animationType = {'fade'}
                        transparent = {true}
